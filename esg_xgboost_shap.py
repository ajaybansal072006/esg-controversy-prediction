"""
============================================================
ESG Controversy Prediction — XGBoost + SHAP
============================================================
...
OUTPUT_DIR = os.path.join(BASE_DIR, "output", "xg_boost")
...
print("=" * 60)
print("  ESG Controversy Prediction — XGBoost + SHAP")
print("=" * 60)
...
#
#  XGBoost handles NaN natively (it learns the best split
#  direction for missing values), so no imputation is strictly
#  required for tree models.  We impute here only to keep the
#  pipeline compatible with any sklearn estimator that does
#  NOT support NaN (e.g. Logistic Regression, SVM).
#
...
#   • GrowthRate (raw)      → industry-year median
# ─────────────────────────────────────────────────────────────
...
# GrowthRate: fill with column median (originally missing in raw data)
...
# ============================================================
# STEP 6: CLASS IMBALANCE + TRAIN XGBOOST
# ============================================================
print("\n[6/9] Training XGBoost model...")

# scale_pos_weight = count(negative) / count(positive)
# Penalises misclassifying the minority controversy class
# more heavily, addressing the ~75/25 class imbalance.
neg = (y_train == 0).sum()
pos = (y_train == 1).sum()
spw = neg / pos
...
# ── Reset X_test index so it aligns with shap_df (0-based) ──
# X_test was sliced from the full DataFrame and carries its
# original integer index (e.g. 3000–10999).  shap_df is always
# built with a fresh 0-based RangeIndex, so .loc[] lookups in
# the company-level explanation loop would raise KeyError.
# Resetting here makes both share the same 0-based index.
X_test  = X_test.reset_index(drop=True)
y_test  = y_test.reset_index(drop=True)
df_test = df[test_mask].reset_index(drop=True)   # for CompanyID / Year lookup

shap_df = pd.DataFrame(shap_values, columns=feature_names)

# ============================================================
# COMPANY-LEVEL RISK DRIVER EXPLANATION
# ============================================================

print("      Building company-level risk explanations...")

risk_prob = model.predict_proba(X_test)[:, 1]

feature_label_map = {
    "Declining_Revenue_Flag": "declining revenue",
    "Margin_Volatility_3Y": "high margin volatility",
    "Negative_Margin_Flag": "negative margin",
    "Margin_vs_Industry": "below-industry profitability",
    "Revenue_vs_Industry": "below-industry revenue",
    "MCap_Collapse_Flag": "market cap collapse",
    "Low_Growth_Low_Margin": "low growth + low margin",
    "Revenue_Surprise": "negative revenue surprise",
    "Profitability_Score": "weak profitability",
    "Valuation_Excess": "valuation excess",
}

explanations = []

# ── FIX: iterate by positional index (0-based) ───────────────
# shap_df, X_test, y_test, and df_test all share the same
# 0-based RangeIndex after the reset_index() calls above,
# so we enumerate over range(len(shap_df)) instead of
# X_test.index (which previously held the original 3000+
# offsets and caused the KeyError).
for i in range(len(shap_df)):
    row = shap_df.iloc[i]

    # positive SHAP -> pushes toward controversy
    positive_contrib = row[row > 0].sort_values(ascending=False)
    top_features = positive_contrib.head(4).index.tolist()

    drivers = [
        feature_label_map.get(f, f.replace("_", " ").lower())
        for f in top_features
    ]

    explanations.append({
        "CompanyID":        df_test.loc[i, "CompanyID"],
        "Year":             df_test.loc[i, "Year"],
        "Risk_Probability": round(risk_prob[i] * 100, 2),
        ...
        "Driver_4": drivers[3] if len(drivers) > 3 else None,
    })

risk_driver_df = pd.DataFrame(explanations)
risk_driver_df.to_csv(os.path.join(OUTPUT_DIR,"company_risk_explanations.csv"), index=False)
print("       Saved: company_risk_explanations.csv")
...
# SAVE MODEL + METRICS
# ============================================================
joblib.dump(model, os.path.join(OUTPUT_DIR,"xgboost_esg_model.pkl"))
print("      ✅ Saved: xgboost_esg_model.pkl")
